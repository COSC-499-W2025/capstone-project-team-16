...
def print_repo_summary(
    proj_name, repo_name, repo_root, repo_authors, repo_contributors,
    branch_count, has_merges, project_type, repo_duration_days, commit_frequency
):
    print("\n[Repository Metadata]")
    print(f" Project:          {proj_name}")
    print(f" Repo Name:        {repo_name}")
    print(f" Repo Root:        {repo_root}")
    print(f" Authors:          {', '.join(sorted(repo_authors)) if repo_authors else 'None'}")
    print(f" Contributors:     {', '.join(sorted(repo_contributors)) if repo_contributors else 'None'}")
...
def _load_filters(filename: str = "extractor_filters.json") -> dict:
    # look for json in SAME folder as this file (src/)
...
def analyze_projects(extracted_data, filters, detailed_data=None, write_csv=True):
    ...
    for proj_name, files in projects.items():
        ...
        # simple score for ranking (will make smarter later)
        ...
        project_summaries.append(
            {
                ...
                # GIT / REPO FIELDS (for advanced mode n reports)
                "repo_name": repo_name,
                "repo_root": repo_root,
